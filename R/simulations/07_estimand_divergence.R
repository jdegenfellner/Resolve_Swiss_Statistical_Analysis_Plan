# =====================================================================
# Internal question, not a paper claim: how far apart can the three
# numbers of Table 4 be?
...
for (i in seq_len(nrow(scen))) {
  ...
  cat(sprintf("cv %.2f | ICC %.2f | gamma %.0f  ->  true PA %.2f / CA %.2f | mixed %.2f, sw %.2f, uw %.2f\n",
...
print(res[c("cv", "rho", "gamma", "bias_mixed_pa", "bias_sw_pa", "bias_uw_ca")],
      row.names = FALSE)
...
fig <- ggplot(pd, aes(factor(cv), est, colour = what, group = what)) +
  ...
  labs(x = "coefficient of variation of practice size", y = "mean estimate",
       colour = NULL,
       title = "How far apart are the three reported quantities?",
