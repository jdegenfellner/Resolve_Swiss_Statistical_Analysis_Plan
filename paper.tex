...
also consistent under misspecification of the working model, and with few
...
score, an individual-level covariate that consumes no cluster-level
...
The protocol specifies a linear mixed model with arm, time and their
interaction, time entered as an ordered categorical variable
\citep{protocol2025}. We fit that model as a supportive analysis, using all
four occasions on which the RMDQ is collected (baseline, 18, 26 and 52
weeks), with a random intercept for practice and a random intercept for
participant nested within practice. The treatment effect at 18 weeks is the
corresponding arm-by-time contrast, and the model also provides the
effects at 26 and 52 weeks, which are secondary.

Two differences from the protocol's wording are worth recording. The protocol's
model statement does not mention a random effect for the practice. Omitting it
would treat patients from the same practice as independent and understate the
standard error, so a practice random effect is carried in every model in this
plan. And with the RMDQ measured before as well as after randomisation, the
baseline-adjusted model \eqref{eq:primary} is the more precise estimator of the
18-week effect than the arm-by-time contrast, which is why it, and not the
repeated-measures model, is the primary analysis \citep{hooper2018,
vickers2001}. Mixed models for repeated measures do behave acceptably in cluster
randomised trials with continuous outcomes missing at random, including in
simulations with as few as five clusters per arm \citep{bell2020}.
