...
and participant. The arms share a common baseline mean by design, which is
what lets the model use the baseline information, and the treatment effect is
estimated with Kenward--Roger degrees of freedom. A supplementary Bayesian
...
January 2026, conditions confirmed fulfilled on 10 April 2026). The trial is
...
violates the variable inclusion principle, under which the interaction
coefficient stops measuring what it should, because the omission imposes a
constraint that is in general false \citep{westfall2020}. Here the omitted
term is the between-arm difference at baseline. The baseline is measured
before allocation, so that difference is zero by design, the constraint is
true rather than assumed, and $\beta_2$ still estimates exactly the 18-week
treatment effect. Imposing the constraint is what lets the model use the
baseline information. This constrained longitudinal formulation
...
pre-randomisation measurement of the outcome in a cluster randomised trial
...
two arms. We constrain them to be equal, which is true by design and recovers
...
primary model will be fitted to those data, and the resulting posterior for
the treatment effect, downweighted to acknowledge that the Australian
comparator was a sham rather than usual physiotherapy, will serve as the
informative prior. The RMDQ at 18 weeks piles up near zero rather than
...
that the benefit exceeds 0 and 2 RMDQ points under each prior. The machinery
matches the Bayesian interim analysis that the protocol specifies
(section~\ref{sec:interim}), so the interim and final analyses are
consistent. The Bayesian analysis is supplementary. No claim of efficacy will
...
2025-00784. Approval with conditions on 20 January 2026, conditions confirmed
fulfilled on 10 April 2026.
